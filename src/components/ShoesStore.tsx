...
const ShoesStore = () => {
  ...
  return (
    <div className="text-center  max-w-full-screen ">
      <header className="sticky top-0 z-50 px-10 py-4 bg-white/80 backdrop-blur-md border-b border-blue-100 shadow-sm flex items-center justify-between">
        ...
        <button
          className="text-white font-medium bg-blue-600 hover:bg-blue-800 cursor-pointer py-2 px-6 flex gap-5 items-center rounded-lg"
          onClick={() => {
            setIsCart(!isCart);
          }}
        >
      ...
      {isCart && (
        <div className="relative">
          <div
            className="inset-0 fixed bg-black/50 z-10"
            onClick={() => {
              setIsCart(false);
            }}
          ></div>
          <CartProduct cart={cart} setIsCart={setIsCart} setCart={setCart} dataShoes={dataShoes}/>
        </div>
      )}
      <ProductList
        data={dataShoes}
